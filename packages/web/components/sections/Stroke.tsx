  ...
  return (
    <section className="w-full max-w-[1200px] mx-auto space-y-6 sm:space-y-8 px-4">
      <div className="space-y-2 text-center sm:text-left mb-6">
        <h2 className="header-sm md:header-md font-bold text-foreground">Stroke Widths</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {strokes.map((s) => (
          ...
          >
            ...
            >
              <div className={`size-8 rounded-md border-primary/50 bg-primary/5 ${s.class}`} />
            </div>
            <div className="text-center space-y-1">
              <p className="text-sm font-bold text-foreground">{s.name}</p>
              <p className="text-[10px] font-mono text-muted-foreground">{s.class}</p>
            </div>
          </div>
        ))}
      </div>
